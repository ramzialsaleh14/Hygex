...
const createDeviceId = async () => {
    const id = uuid.v4();
    if (!id) {
        return `backup-${Date.now()}-${Math.random().toString(36).substring(2)}`;
    }
    return uuid.v4();
};

export const registerUserToken = async (navigation) => {
    let devId = await createDeviceId();
    console.log('Generated device ID:', devId);

    const userID = await Commons.getFromAS("userID");
    const curDevId = await Commons.getFromAS("devId");
    if (curDevId == null || curDevId == "" || curDevId == undefined) {
